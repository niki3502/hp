...
for(i;i<strlen(base);i++){
  ...
  if(base[i]=='+'){i+=1+base[i+1]-'0';continue;}
    ...
    if(base[i]=='A'||base[i]=='a')a+=pow(10,-(baq[j]-33+mq[i]-33)/10);
    if(base[i]=='T'||base[i]=='t')t+=pow(10,-(baq[j]-33+mq[i]-33)/10);
    if(base[i]=='G'||base[i]=='g')g+=pow(10,-(baq[j]-33+mq[i]-33)/10);
    if(base[i]=='C'||base[i]=='c')c+=pow(10,-(baq[j]-33+mq[i]-33)/10);
    j++;
